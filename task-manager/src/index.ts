...
const viewTasks = (): void => {
 ...
 console.log(`${index + 1}. ${status} ${title} ${categoryText}
    ${chalk.dim(`(created: ${date})`)}`);  
    ...
    const dueDate = task.dueDate
      ...
      : 'No due date'; // Show 'No due date' if no dueDate is set
    // Apply color to priority
    let coloredPriority: string;
    switch (task.priority) {
      case 'low':
        coloredPriority = chalk.green(task.priority);
        break;
      case 'medium':
        coloredPriority = chalk.yellow(task.priority);
        break;
      case 'high':
        coloredPriority = chalk.red(task.priority);
        break;
      default:
        coloredPriority = task.priority;
    }
    // Output task
    console.log(
        `${index + 1}. ${status} ${title} ${chalk.dim(`(created: ${date})`)} [Priority: ${coloredPriority}]`
      );
       // Display due date
    console.log(chalk.dim(`   Due Date: ${dueDate}`));
...
const getCategoryColor = (category: string): typeof chalk=> {
    const colors: Record<Category, typeof chalk> = {
    work: chalk.blue,
...
chalk.white(task.title);
     ...
     const answers: AddTaskAnswers = await inquirer.prompt([
        {
            ...
            },
            {
              ...
              },
              {
                type: 'input',
                name: 'dueDate',
                message: 'Enter due date (MM-DD-YY) (optional):',
 ...
 const task = new Task(answers.title.trim(), answers.description.trim(), answers.priority, answers.dueDate.trim() || undefined, answers.category);
